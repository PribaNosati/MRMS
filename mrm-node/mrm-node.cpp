...
*/
Mrm_node::Mrm_node(ESP32CANBus *esp32CANBusSingleton, BluetoothSerial * hardwareSerial) {
...
{
	if (nextFree >= MAX_MRM_NODE)
		error("Too many mrm-node");

	switch (nextFree) {
	case 0:
		idIn[nextFree] = CAN_ID_NODE0_IN;
		idOut[nextFree] = CAN_ID_NODE0_OUT;
		break;
	case 1:
		idIn[nextFree] = CAN_ID_NODE1_IN;
		idOut[nextFree] = CAN_ID_NODE1_OUT;
		break;
	default:
		error("Too many mrm-node.");
	}
...
*/
void Mrm_node::print(const char* fmt, ...) {
	va_list argp;
	va_start(argp, fmt);
	vprint(fmt, argp);
	va_end(argp);
...
*/
void Mrm_node::devicesScan(bool verbose) {
...
*/
void Mrm_node::vprint(const char* fmt, va_list argp) {
