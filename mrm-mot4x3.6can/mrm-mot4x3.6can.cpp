...
*/
Mrm_mot4x3_6can::Mrm_mot4x3_6can(ESP32CANBus *esp32CANBusSingleton, BluetoothSerial * hardwareSerial) {
...
{
	if (nextFree >= MAX_MRM_MOT4X3_6CAN)
		error("Too many Mrm_mot4x3_6can");

	switch (nextFree) {
	case 0:
		idIn[nextFree] = CAN_ID_MOT4X3_6_MOTOR0_IN;
		idOut[nextFree] = CAN_ID_MOT4X3_6_MOTOR0_OUT;
		break;
	case 1:
		idIn[nextFree] = CAN_ID_MOT4X3_6_MOTOR1_IN;
		idOut[nextFree] = CAN_ID_MOT4X3_6_MOTOR1_OUT;
		break;
	case 2:
		idIn[nextFree] = CAN_ID_MOT4X3_6_MOTOR2_IN;
		idOut[nextFree] = CAN_ID_MOT4X3_6_MOTOR2_OUT;
		break;
	case 3:
		idIn[nextFree] = CAN_ID_MOT4X3_6_MOTOR3_IN;
		idOut[nextFree] = CAN_ID_MOT4X3_6_MOTOR3_OUT;
		break;
	default:
...
@param right Speed
*/
void Mrm_mot4x3_6can::go(int8_t leftSpeed, int8_t rightSpeed) {
...
@return - name
*/
String Mrm_mot4x3_6can::name(uint8_t motorNumber) {
...
*/
void Mrm_mot4x3_6can::print(const char* fmt, ...) {
	va_list argp;
	va_start(argp, fmt);
	vprint(fmt, argp);
	va_end(argp);
...
*/
void Mrm_mot4x3_6can::devicesScan(bool verbose) {
...
*/
void Mrm_mot4x3_6can::setSpeed(uint8_t motorNumber, int8_t speed) {
	if (nextFree >= MAX_MRM_MOT4X3_6CAN)
...
*/
void Mrm_mot4x3_6can::test(BreakCondition breakWhen)
{
	const uint16_t PAUSE_MS = 50;
	...
	while (goOn) {
		for (volatile uint8_t motorNumber = 0; motorNumber < nextFree; motorNumber++) {
			...
			for (int16_t i = 0; i <= 127 && goOn; i += STEP) {
				//blink();
				if ((*breakWhen)())
					goOn = false;
			...
			for (int16_t i = 127; i >= -127 && goOn; i -= STEP) {
				//blink();
				if ((*breakWhen)())
					goOn = false;
				setSpeed(motorNumber, i);
			...
			for (int16_t i = -127; i < 0 && goOn; i += STEP) {
				//blink();
				if ((*breakWhen)())
					goOn = false;
				setSpeed(motorNumber, i);
...
*/
void Mrm_mot4x3_6can::vprint(const char* fmt, va_list argp) {
