...
void VL53L1Xs::begin(boolean verbose) {
	for (uint8_t i = 0; i < nextFree; i++) {
		...
		if (VL53L1_GetDeviceInfo(pDev[i], &DeviceInfo) != VL53L1_ERROR_NONE)
			errorVL(i, "Info");
		if (verbose) {
			print("VL53L1X " + (String)DeviceInfo.Name + ", type: " + (String)DeviceInfo.Type + ", id: " + (String)(int)DeviceInfo.ProductId + 
...
*/
void VL53L1Xs::print(String message, bool eol) {
