...
*/
Mrm_mot4x10::Mrm_mot4x10(ESP32CANBus *esp32CANBusSingleton, BluetoothSerial * hardwareSerial) {
...
{
	if (nextFree >= MAX_MRM_MOT4X10)
		error("Too many Mrm_mot4x10");

	switch (nextFree) {
	case 0:
		idIn[nextFree] = CAN_ID_MOT4X10_MOTOR0_IN;
		idOut[nextFree] = CAN_ID_MOT4X10_MOTOR0_OUT;
		break;
	case 1:
		idIn[nextFree] = CAN_ID_MOT4X10_MOTOR1_IN;
		idOut[nextFree] = CAN_ID_MOT4X10_MOTOR1_OUT;
		break;
	case 2:
		idIn[nextFree] = CAN_ID_MOT4X10_MOTOR2_IN;
		idOut[nextFree] = CAN_ID_MOT4X10_MOTOR2_OUT;
		break;
	case 3:
		idIn[nextFree] = CAN_ID_MOT4X10_MOTOR3_IN;
		idOut[nextFree] = CAN_ID_MOT4X10_MOTOR3_OUT;
		break;
	default:
...
@param right Speed
*/
void Mrm_mot4x10::go(int8_t leftSpeed, int8_t rightSpeed) {
...
@return - name
*/
String Mrm_mot4x10::name(uint8_t motorNumber) {
...
*/
void Mrm_mot4x10::print(const char* fmt, ...) {
	va_list argp;
	va_start(argp, fmt);
	vprint(fmt, argp);
	va_end(argp);
...
*/
void Mrm_mot4x10::devicesScan(bool verbose) {
...
*/
void Mrm_mot4x10::setSpeed(uint8_t motorNumber, int8_t speed) {
	if (nextFree >= MAX_MRM_MOT4X10)
...
*/
void Mrm_mot4x10::test(BreakCondition breakWhen)
{
...
*/
void Mrm_mot4x10::vprint(const char* fmt, va_list argp) {
