...
*/
Mrm_bldc4x2_5::Mrm_bldc4x2_5(ESP32CANBus *esp32CANBusSingleton, BluetoothSerial * hardwareSerial) {
...
{
	if (nextFree >= MAX_BLDC4X2_5)
		error("Too many Mrm_bldc4x2_5");

	switch (nextFree) {
	case 0:
		idIn[nextFree] = CAN_ID_BLDC4X2_5_MOTOR0_IN;
		idOut[nextFree] = CAN_ID_BLDC4X2_5_MOTOR0_OUT;
		break;
	case 1:
		idIn[nextFree] = CAN_ID_BLDC4X2_5_MOTOR1_IN;
		idOut[nextFree] = CAN_ID_BLDC4X2_5_MOTOR1_OUT;
		break;
	case 2:
		idIn[nextFree] = CAN_ID_BLDC4X2_5_MOTOR2_IN;
		idOut[nextFree] = CAN_ID_BLDC4X2_5_MOTOR2_OUT;
		break;
	case 3:
		idIn[nextFree] = CAN_ID_BLDC4X2_5_MOTOR3_IN;
		idOut[nextFree] = CAN_ID_BLDC4X2_5_MOTOR3_OUT;
		break;
	default:
...
@param right Speed
*/
void Mrm_bldc4x2_5::go(int8_t leftSpeed, int8_t rightSpeed) {
...
@return - name
*/
String Mrm_bldc4x2_5::name(uint8_t motorNumber) {
...
*/
void Mrm_bldc4x2_5::print(const char* fmt, ...) {
	va_list argp;
	va_start(argp, fmt);
	vprint(fmt, argp);
	va_end(argp);
...
*/
void Mrm_bldc4x2_5::devicesScan(bool verbose) {
...
*/
void Mrm_bldc4x2_5::setSpeed(uint8_t motorNumber, int8_t speed) {
	if (nextFree >= MAX_BLDC4X2_5)
...
*/
void Mrm_bldc4x2_5::test(BreakCondition breakWhen)
{
...
*/
void Mrm_bldc4x2_5::vprint(const char* fmt, va_list argp) {
