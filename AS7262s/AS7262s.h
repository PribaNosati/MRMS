...
#define MAX_MRM_COL 4 //Maximum number of sensors. 
...
class AS7262s
{
	...
	i2c_t3 *i2c[MAX_MRM_COL];// Every sensor can use a different I2C bus
...
public:
	...
	@param hardwareSerial - Serial, Serial1, Serial2,... - an optional serial port, for example for Bluetooth communication. Example:
		AS7262s(&Serial2);
	*/
	AS7262s(HardwareSerial * hardwareSerial = 0);
	...
	@param gain
	*/
	void add(i2c_t3 *i2cBus = &Wire, byte address = 0x49, uint8_t integrationTime = 50, uint8_t dataConversionType = 2, uint8_t gain = 3);
