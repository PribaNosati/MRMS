...
*/
void IMUBoschBNO055::add(bool defaultI2CAddress) {
	if (nextFree >= MAX_IMU_BOSCH_BNO055_SENSORS)
...
@return - Calibration
*/
uint8_t IMUBoschBNO055::accelerationCalibration() {
	uint8_t acc;
...
@return - Calibration
*/
uint8_t IMUBoschBNO055::gyroCalibration() {
	uint8_t cal;
...
@return - Calibration
*/
uint8_t IMUBoschBNO055::magneticCalibration() {
	uint8_t cal;
...
*/
void IMUBoschBNO055::print(String message, bool eol) {
	if (eol) {
...
@return - Calibration
*/
uint8_t IMUBoschBNO055::systemCalibration() {
	uint8_t cal;
...
*/
void IMUBoschBNO055::test(BreakCondition breakWhen) {
	char buffer[14];
	while (breakWhen == 0 || !(*breakWhen)()) {
		for (int i = 0; i < nextFree; i++) {
			sprintf(buffer, "Y:%3i P:%3i R:%3i", (int)round(heading()), (int)round(pitch()), (int)round(roll()));
			print(buffer);
		}
		print("", true);
		delay(200);
...
*	\return: communication result
*/
s32 IMUBoschBNO055::bno055_data_readout_template(void)
{
...
void IMUBoschBNO055::bno055Initialize(bool defaultI2CAddress)
{
	s32 comres = BNO055_ERROR;
...
*/
IMUBoschBNO055::IMUBoschBNO055(HardwareSerial * hardwareSerial) {
	serial = hardwareSerial;
	nextFree = 0;
}

IMUBoschBNO055::~IMUBoschBNO055(){}